...
class AWRJointTrainer(ActorCriticJointTrainer):
  ...
  def joint_loss(self):
    ...
    @tl.layer
    def awr_loss(logps, values, returns, actions):
      advantage = returns - values
      l2_value_loss = (returns - values)**2 * self._value_loss_coeff
      awr_weights = jnp.minimum(jnp.exp(advantage / self._beta), self._w_max)
      xent = -1.0 * self._policy_dist.log_probs(logps, actions)
      policy_loss = jnp.sum(xent * awr_weights) / jnp.sum(awr_weights)
